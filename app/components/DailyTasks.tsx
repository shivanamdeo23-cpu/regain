...
// --- Types
 type Task = {
  id: string;
  label: string;
  evidence: string; // short evidence-based blurb
  category: "Nutrition" | "Exercise" | "Lifestyle";
};

 type TasksByCategory = Record<Task["category"], Task[]>;

// --- Config: tasks (hardcoded for MVP; later load from CMS/user profile)
...
// --- Storage keys (versioned for safe future migrations)
const STORAGE_KEY = "dailyTasks_state_v1"; // { date: "YYYY-MM-DD", completed: string[] }
const STREAK_KEY = "dailyTasks_streak_v1"; // { lastDate: string, streak: number }

// --- Helpers
const fmtDate = (d: Date) => d.toISOString().slice(0, 10); // YYYY-MM-DD (local conversion below)

function todayLocal(): string {
  // Convert to local yyyy-mm-dd by adjusting timezone offset
  const now = new Date();
...
export default function DailyTasks() {
  ...
  // Initialize + handle daily reset
  useEffect(() => {
    const t = todayLocal();
    const s = loadState();
    if (s.date !== t) {
      // New day → reset completed, but first maybe update streak if yesterday was full completion
      ...
      if (allDoneYesterday) {
        if (streakObj.lastDate) {
          ...
          const continued = gap === 1 || gap === 0; // tolerate same-day writes
          saveStreak({ lastDate: prevDate, streak: continued ? streakObj.streak + 1 : 1 });
        } else {
          saveStreak({ lastDate: prevDate, streak: 1 });
        }
      } else {
        // If missed a full completion day, and a day has passed, break the streak
        if (streakObj.lastDate) {
  ...
  function toggleTask(taskId: string) {
    if (!state) return;
    // 1x per day cap → if already completed, do nothing
    if (state.completed.includes(taskId)) return;
    const next = { ...state, completed: [...state.completed, taskId] };

    // If this click completes the last remaining task → update streak live
...
// Create file: app/daily/page.tsx
// import DailyTasks from "@/components/DailyTasks";
// export default function Page() { return <DailyTasks /> }
